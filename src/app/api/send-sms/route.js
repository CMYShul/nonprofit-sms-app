...
export async function POST(request) {
  ...
  try {
    ...
    // Security: Limit recipients and message length to prevent resource exhaustion/abuse
    if (recipients.length > 50) {
      return NextResponse.json({ error: "Cannot send to more than 50 recipients at once" }, { status: 400 });
    }

    if (message.length > 1000) {
      return NextResponse.json({ error: "Message content is too long (max 1000 characters)" }, { status: 400 });
    }
